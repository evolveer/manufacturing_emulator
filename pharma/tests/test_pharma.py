"""
Unit tests for Pharma Batch Execution Simulator domain logic and services.
"""
...
def test_execution_and_review():
    ...
    batch = batch_service.create_batch(order.order_id, order.product_code, order.product_name, order.site, order.quantity, order.unit, "RCP-TABLET-001")
    ...
    execution_service.capture_parameters(batch.batch_id, execs[0].step_id, {"operator_id": "OP-001", "clearance_result": "Pass"}, "OP-001", "RCP-TABLET-001")
    ...
    assert "critical deviation(s) remain open" in decision.comment or "mandatory step(s) not completed" in decision.comment
