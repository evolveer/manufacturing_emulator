#!/usr/bin/env python3
"""
Weekly Manufacturing Report Generator

Fetches live data from MES and PCS and writes a Markdown report to disk.
All service URLs are read from environment variables (or .env file) so
the script works in any deployment without code changes (fixes issue #2).
"""
import requests
import json
import datetime
...
# Load .env from project root (fixes issue #2 – no more hardcoded localhost URLs)
_project_root = Path(__file__).parent
load_dotenv(_project_root / '.env')

# Service URLs – override via environment variables for non-localhost deployments
MES_URL = os.environ.get('INTERFACE_MES_URL', 'http://localhost:5000/api/mes')
PCS_URL = os.environ.get('PCS_URL', 'http://localhost:5003/api/v1')
_MES_DIRECT_URL = os.environ.get('MES_URL', 'http://localhost:5002/api/v1')
REPORT_DIR = os.environ.get('REPORT_DIR', str(_project_root / 'reports'))

# Configure logging (fixes issue #12 – use logger instead of print)
...
def generate_report():
    ...
    # Fetch MES data
    mes_status = fetch_data(f"{_MES_DIRECT_URL}/status")
